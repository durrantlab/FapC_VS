...
## Analysis
[check_output](/study/b02-dock-div-set/analysis/check_output.py): checks if the output of gnina was sucessful
[concat_sdf_files](/study/b02-dock-div-set/analysis/concat_sdf_files.py): takes in a list of SDFs, each with different conformers of the same molecule, and puts them into N different concatted SDFs together. Allows for 1 docking job to do much more.
[create_gnina_job_list](/study/b02-dock-div-set/analysis/create_gnina_job_list.py): takes in all boxes, ligands and the protein and creates gnina inputs to dock every ligand to every box.
- job_list.txt: holds the arguments for each job. Each line is a different job.
- run_gnina.sh: bash script that submits the jobs
[dock.slurm](/study/b02-dock-div-set/README.md): batch job specification for the gnina jobs
