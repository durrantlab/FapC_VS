...
## Analysis
[concat_sdf_files](analysis/concat_sdf_files.py): takes in a list of SDFs, each with different conformers of the same molecule, and puts them into N different concatted SDFs together. Allows for 1 docking job to do much more.
[create_gnina_job_list](analysis/create_gnina_job_list.py): takes in all boxes, ligands and the protein and creates gnina inputs to dock every ligand to every box.
- job_list.txt: holds the arguments for each job. Each line is a different job.
- run_gnina.sh: bash script that submits the jobs
[dock.slurm](analysis/dock.slurm): batch job specification for the gnina jobs
