...
def main(main_disabled_pharmit_dir: Path, main_pharmit_output_dir: Path, 
        pharm_search_base: Path, iterative_pharm_loc: Path, max_ret_mol: int = 2000, DIR_SCRIPT: Path = DIR_SCRIPT):
    ...
    pharmit_input_jsons: list[Path] = [item for item in main_disabled_pharmit_dir.iterdir() if item.is_file() and item.suffix == ".json"]
    ...
    with open(batch_script_file, "w") as f:
        f.write(f"sbatch --array=0-{len(pharmit_input_jsons)-1} --export=ALL pharm_search.slurm\n")
    
    # create the slurm to go to correct python
    with open(pharm_search_base, "r") as f:
        slurm_text: list[str] = f.read().strip().split("\n")
    slurm_text[23] = f'bash -c "pixi run python -u {str(iterative_pharm_loc)} $ARGS" | tee "$LOG"'
...
if __name__ == "__main__":
    # inputs
    disabled_pharmit_dir: Path = (DIR_STUDY / "064-validate-pharm-exp" / "data" / "visual_inspect").resolve()
    ...
    iterative_pharm_loc: Path = (DIR_STUDY / "033-pharm-search-top-div" / "analysis" / "iterative_pharm.py").resolve()
    
    main(disabled_pharmit_dir, pharmit_output_dir, pharm_search_base, iterative_pharm_loc, 5000)
